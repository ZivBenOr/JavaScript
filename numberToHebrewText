 ...
 */
const numberToHebrewText = function (number) {
    ...
    const Num = (index, index2) => index2 ? Number(n[index] + n[index2]) : Number(n[index])
    const And = (num) => num && temp && 'ו' + num || num
    const Hunbreds = (i) => d[Num(i)] || (a[Num(i)] && (a[Num(i)] + 'מאות ')) || ''
    const Tens = (i) => ((temp = str), Num(i) > 0 ? (And(a[Num(i)]) || ((temp = b[n[i][0]]) + And(a[n[i][1]]))) : '')
